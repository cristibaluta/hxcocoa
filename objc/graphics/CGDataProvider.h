...
/* Return the CFTypeID for CGDataProviderRefs. */
	...
	@:c public static function CFTypeID CGDataProviderGetTypeID(void)
...
/* Create a sequential-access data provider using `callbacks' to provide the
   data. `info' is passed to each of the callback functions. */
	...
	@:c public static function CGDataProviderRef CGDataProviderCreateSequential(void *info,
...
/* Create a direct-access data provider using `callbacks' to supply `size'
   bytes of data. `info' is passed to each of the callback functions. */
	...
	@:c public static function CGDataProviderRef CGDataProviderCreateDirect(void *info, off_t size,
...
/* Create a direct-access data provider using `data', an array of `size'
   ...
   passed `info' as its first argument. */
	...
	@:c public static function CGDataProviderRef CGDataProviderCreateWithData(void *info,
...
/* Create a direct-access data provider which reads from `data'. */
	...
	@:c public static function CGDataProviderRef CGDataProviderCreateWithCFData(CFDataRef data)
  CG_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_2_0);

/* Create a data provider reading from `url'. */

	@:c public static function CGDataProviderRef CGDataProviderCreateWithURL(CFURLRef url)
  CG_AVAILABLE_STARTING(__MAC_10_0, __IPHONE_2_0);

/* Create a data provider reading from `filename'. */

	@:c public static function CGDataProviderRef
...
/* Equivalent to `CFRetain(provider)', but doesn't crash (as CFRetain does)
   if `provider' is NULL. */
	...
	@:c public static function CGDataProviderRef CGDataProviderRetain(CGDataProviderRef provider)
  CG_AVAILABLE_STARTING(__MAC_10_0, __IPHONE_2_0);

/* Equivalent to `CFRelease(provider)', but doesn't crash (as CFRelease
   does) if `provider' is NULL. */

	@:c public static function void CGDataProviderRelease(CGDataProviderRef provider)
  CG_AVAILABLE_STARTING(__MAC_10_0, __IPHONE_2_0);
...
/* Return a copy of the data specified by provider. Returns NULL if a
   ...
   underlying data is too large to fit in memory). */
	...
	@:c public static function CFDataRef CGDataProviderCopyData(CGDataProviderRef provider)
