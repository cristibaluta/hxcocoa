...
#if __has_attribute(swift_returns_inner_pointer)
#define NS_RETURNS_INNER_POINTER __attribute__((swift_returns_inner_pointer))
#else
...
#if __has_attribute(swift_arc_weak_reference_unavailable)
#define NS_AUTOMATED_REFCOUNT_WEAK_UNAVAILABLE __attribute__((swift_arc_weak_reference_unavailable))
#else
...
#if __has_attribute(swift_requires_property_definitions)
#define NS_REQUIRES_PROPERTY_DEFINITIONS __attribute__((swift_requires_property_definitions)) 
#else
...
#if __has_attribute(swift_precise_lifetime)
#define NS_VALID_UNTIL_END_OF_SCOPE __attribute__((swift_precise_lifetime))
#else
...
#if __has_attribute(swift_root_class)
#define NS_ROOT_CLASS __attribute__((swift_root_class))
#else
...
// This macro is to be used by system frameworks to support the weak linking of classes. Weak linking is supported on iOS 3.1 and Mac OS X 10.6.8 or later.
    ...
    ((__has_feature(swift_weak_class) || \
...
#if (__cplusplus && __cplusplus >= 201103L && (__has_extension(cxx_strong_enums) || __has_feature(swift_fixed_enum))) || (!__cplusplus && __has_feature(swift_fixed_enum))
