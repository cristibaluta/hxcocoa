...
// GLKBaseEffect 
//
// GLKBaseEffect is designed to simplify configuration of OpenGLES for visual effects common
// to many OpenGLES applications today.  GLKBaseEffect is compatible with OpenGLES 2.0 
// or greater.  An OpenGLES 2.0+ context must be created and made current prior to instantiating
// and initializing GLKBaseEffect objects.
//
// GLKBaseEffect is designed to freely interoperate with custom OpenGL 2.0 ES code.  It is also
...
// 
// As with all OpenGL ES 2.0 drawing GLKBaseEffect is OpenGLES GLSL program based and,
// with the binding of its underlying GLSL program, it modifies GL_CURRENT_PROGRAM state setting
// implicitly when GLKBaseEffect:prepareToDraw is called.  For performance reasons GL_CURRENT_PROGRAM is not
// saved and restored by GLKBaseEffect so clients of this class must set/save/restore the GL_CURRENT_PROGRAM
// before and/or after a call to GLKBaseEffect:prepareToDraw as is applicable for their application.
//
// If texturing properties have been specified and are enabled, GLKBaseEffect will also modify
// the OpenGLES state element GL_TEXTURE_BINDING_2D.  This state must also be handled in the manner
...
//
//      (1) Alloc/Init GLKBaseEffect object
//
...
//          by calling prepareToDraw().  Draw the model with the effect
//
